...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, episode_ids, dataset_dir, policy_config, norm_stats, arm_delay_time):
        ...
        if policy_config['policy_class'] == "ACT":
            ...
            self.use_qdiff = policy_config['use_qdiff']
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            left_states = np.concatenate((left_states, np.array(left_qpos - left_states_init)),
                                         axis=0) if self.use_qdiff else left_states
            ...
            right_states = np.concatenate((right_states, np.array(right_qpos - right_states_init)),
                                          axis=0) if self.use_qdiff else right_states
...
def get_IO_for_norm(qpos, eef, qvel, effort, action, policy_config):
    if policy_config['policy_class'] == "ACT":
        ...
        use_qdiff = policy_config['use_qdiff']
    ...
    if use_qdiff:
        ...
        for idx in range(qpos.shape[0]):
            left_states_diff.append([a - b for a, b in zip(left_qpos[idx], left_qpos[0])])
        left_states_diff = np.array(left_states_diff)

        right_states_diff = []
        if policy_config['robot'] != 'Single':  # right
            for idx in range(qpos.shape[0]):
                right_states_diff.append([a - b for a, b in zip(right_qpos[idx], right_qpos[0])])
            right_states_diff = np.array(right_states_diff)

    left_states = np.concatenate((left_states, qvel[:, :joints_dim]),
                                 axis=1) if use_qvel else left_states
    left_states = np.concatenate((left_states, effort[:, joints_dim - 1:joints_dim]),
                                 axis=1) if use_effort else left_states
    left_states = np.concatenate((left_states, left_states_diff),
                                 axis=1) if use_qdiff else left_states

    right_states = np.concatenate((right_states, qvel[:, joints_dim:joints_dim * 2]),
                                  axis=1) if use_qvel else right_states
    right_states = np.concatenate((right_states, effort[:, joints_dim * 2 - 1:joints_dim * 2]),
                                  axis=1) if use_effort else right_states
    right_states = np.concatenate((right_states, right_states_diff),
                                  axis=1) if use_qdiff else right_states
